...
import com.kingyu.rlbird.game.pathery.Pathery;
...
public final class TrainPathery {
    private static final Logger logger = LoggerFactory.getLogger(TrainPathery.class);
...
//    static RlEnv.Step[] batchSteps;
    ...
    public static void train(Arguments arguments, Model model) {
        ...
        Pathery game = new Pathery(NDManager.newBaseManager(), batchSize, REPLAY_BUFFER_SIZE);

            DefaultTrainingConfig config = setupTrainingConfig();
            try (Trainer trainer = model.newTrainer(config)) {
            	// TODO 170w x 101h
//            	.
                trainer.initialize(new Shape(batchSize, 4, 170, 101));
                ...
                callables.add(new PatheryCallable(game, agent, model, training));
//                if(training) {
...
//                }
    ...
    public static void test(Model model) {
        Pathery game = new Pathery(NDManager.newBaseManager(), 1, 1);
        DefaultTrainingConfig config = setupTrainingConfig();
        try (Trainer trainer = model.newTrainer(config)) {
            RlAgent agent = new QAgent(trainer, REWARD_DISCOUNT);
            while (true) {
...
//    }
    ...
    private static class PatheryCallable implements Callable<Object> {
        private final Pathery game;
        private final RlAgent agent;
        private final Model model;
        private final boolean training;

        public PatheryCallable(Pathery game, RlAgent agent, Model model, boolean training) {
            this.game = game;
            this.agent = agent;
        ...
        @Override
        public Object call() {
        	int trainSteps = game.getTrainStep();
            while (trainSteps < EXPLORE) {
                Step[] batchSteps = game.runEnvironment(agent, training);
	                if(training) {
	                this.agent.trainBatch(batchSteps);
//	                FlappyBird.trainStep++;
	                trainSteps++;
	                if (trainSteps > 0 && trainSteps % SAVE_EVERY_STEPS == 0) {
	                    try {
							model.save(Paths.get(Constant.MODEL_PATH), "dqn-" + trainSteps);
						} catch (IOException e) {
